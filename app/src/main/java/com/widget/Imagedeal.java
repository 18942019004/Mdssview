...
/**
 * 根据URL异步获取图片显示在控件上 传入参数flag：为0时为普�?浏览图片，为1时为从相机或者相册获取图片，�??时为从相机或者相册获取缩放图�??
 *
 * @author
 */
public class Imagedeal
{
    ...
    public static final int COPHOTOHRAPH = 2;// 缩放拍照
    public static final int PHOTOZOOM = 3; // 相册
    public static final int COPHOTOZOOM = 4; // 缩放相册
    public static final int PHOTORESOULT = 5;// 处理后的照片
    // 照片处理的宽高比例以及输出大�??
    ...
    Handler h = new Handler()
    {
        ...
        public void handleMessage(Message msg)
        {
            ...
            if (msg.what == 0x11)
            {
                matrix.postTranslate(msg.getData().getFloat("deltax"),msg.getData().getFloat("deltay"));
                img.setImageMatrix(matrix);
                img.invalidate();
                if (Math.abs(msg.getData().getFloat("deltax")) == ratio * seed
                        && Math.abs(msg.getData().getFloat("deltay")) == ratio1 * seed)
                {
                    setMess(con(msg.getData().getFloat("sum") - seed,msg.getData().getFloat("deltax")),
                            con(msg.getData().getFloat("sum") - seed,msg.getData().getFloat("deltay")),msg);
                }
            }
    ...
    public void choose(final int flag,final Activity activity,final UriCallback callback)
    {
        if (flag != 0)
        {
            new AlertDialog.Builder(activity).setTitle("选择你想上传的照").setItems(new String[]
                    {"相机拍摄","手机相册"},new DialogInterface.OnClickListener()
            {
                public void onClick(DialogInterface dialog,int which)
                {
                    switch (which)
                    {
                        case 0:
                            ...
                            ;
                            Uri photoUri = activity.getContentResolver()
                                    .insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI,values);
                            callback.Load(photoUri);
                            m_uri = photoUri;
                            Intent takephoto = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
                            takephoto.putExtra(MediaStore.EXTRA_OUTPUT,photoUri);
                            activity.startActivityForResult(takephoto,requestCode1);
                            break;
                        case 1:
                            Intent i = new Intent(Intent.ACTION_PICK,
                                    MediaStore.Images.Media.EXTERNAL_CONTENT_URI);
                            activity.startActivityForResult(i,requestCode2);
                            break;
                        default:
                            break;
                    }
                    ...
                    if (scale == 1)
                    {
                        ...
                        if (drf != null)
                        {
                            ...
                            if (rf != null)
                            {
                                ...
                                // float sc =(s2- s1) / s1;
                                Log.d(TAG,
                                        "scale1=" + s1 + ",scale2=" + s2 + ",getWidth:" + v.getWidth() + ",getHeight:"
                                                + v.getWidth() + ",rf getWidth:" + rf.getWidth() + ",rf getHeight:"
                                                + rf.getHeight());
                                matrix = new Matrix();
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mode == DRAG)
                {
                    if (scale > 1)
                    {
                        int w = img.getWidth();
                        int h = img.getHeight();
                        Log.d("position","rectF:" + rectF.toString() + ",getWidth:" + w + ",getHeight:" + h);
                        matrix.set(savedMatrix);
...
//        L.e("scale result:" + ret + " scaleType:" + img.getScaleType());
        return true;
    }
    ...
    public void center(boolean horizontal,boolean vertical,boolean fl,ImageView img)
    {
        this.img = img;
        ...
        if (fl == true)
        {
            ...
            {
                ratio = (float) Math.cos(Math.atan2(Math.abs(deltaY),Math.abs(deltaX)));
            }
            ratio1 = (float) Math.sin(Math.atan2(Math.abs(deltaY),Math.abs(deltaX)));
